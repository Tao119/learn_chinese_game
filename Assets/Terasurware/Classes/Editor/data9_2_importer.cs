...
public class data9_2_importer : AssetPostprocessor {
	private static readonly string filePath = "Assets/data9_2.xlsx";
	private static readonly string exportPath = "Assets/data9_2.asset";
	...
	static void OnPostprocessAllAssets (string[] importedAssets, string[] deletedAssets, string[] movedAssets, string[] movedFromAssetPaths)
	{
		foreach (string asset in importedAssets) {
			...
			data9_2 data = (data9_2)AssetDatabase.LoadAssetAtPath (exportPath, typeof(data9_2));
			if (data == null) {
				data = ScriptableObject.CreateInstance<data9_2> ();
			...
			using (FileStream stream = File.Open (filePath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite)) {
				...
				foreach(string sheetName in sheetNames) {
					...
					data9_2.Sheet s = new data9_2.Sheet ();
					s.name = sheetName;
					...
					for (int i=1; i<= sheet.LastRowNum; i++) {
						...
						data9_2.Param p = new data9_2.Param ();
